...
codegen_response_t *codegen_file(char* filename, int start_offset, int duration, int tag) {
    ...
    if (pAudio.get() == NULL) { // Unable to decode!
        ...
        sprintf(output,"{\"error\":\"could not create decoder\", \"filename\":\"%s\"}",
            tag,
    ...
    if (numSamples < 1) {
        char* output = (char*) malloc(16384);
        sprintf(output,"{\"error\":\"could not decode\", \"filename\":\"%s\"}",
            tag,
            escape(filename).c_str());
        response->error = output;
        return response;
...
char *make_json_string(codegen_response_t* response) {
	...
	sprintf(output,"{\"filename\":\"%s\", \"samples_decoded\":%d, "
		"\"given_duration\":%d, \"start_offset\":%d, \"codever\":%2.2f, "
		"\"codegen_time\":%2.6f, \"decode_time\":%2.6f, \"fp_count\":%d, \"fp\":\"%s\"}",
